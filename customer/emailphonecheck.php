...
$pin = ($_POST['pin']);
	...
	if(count($errors) == 0){
		$check = "SELECT * FROM customer WHERE email='".$email."' AND phoneNo='".$phoneNo."'";
		$result = mysqli_query($conn,$check);

		if (mysqli_num_rows($result)==1){
			
				// header('location:custemailotp.php?email='.$email);
				// }
				// else
				// {
				// 	array_push($errors, "Does not exist!");
				// }
				$check2="SELECT * FROM customer WHERE pin='".$pin."'";
...
// 			if (mysqli_num_rows($result)==1){
// 				$check2="SELECT * FROM customer WHERE secretpin='".$pin."'";
// 				$result2 = mysqli_query($conn,$check2);
// 				if(mysqli_num_rows($result2)==1){
// 					// log user in
// 				header('location:forgotpasscust.php?email='.$email);
// 				}elseif($pin!=$result);{
// 					array_push($errors, "Wrong pin!");
// 				}
// 			}else{
