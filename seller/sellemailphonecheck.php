...
$pin = ($_POST['pin']);
	...
	if(count($errors) == 0){
		$check = "SELECT * FROM seller WHERE email='".$email."' AND phoneNo='".$phoneNo."'";
		$result = mysqli_query($conn,$check);

		if (mysqli_num_rows($result)==1){
		
				$check2="SELECT * FROM seller WHERE pin='".$pin."'";
				$result2 = mysqli_query($conn,$check2);
				if(mysqli_num_rows($result2)==1){
					// log user in
				header('location:sellerresetpass.php?email='.$email);
				}elseif($pin!=$result);{
