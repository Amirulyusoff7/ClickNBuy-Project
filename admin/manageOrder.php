             ...
             <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="orderHeader">
                        <a href="manageOrder.php" class="link-primary">All</a>&nbsp&nbsp
                        <a href="unpaidOrder.php" class="link-primary">Unpaid</a>&nbsp&nbsp
                        <a href="toShipOrder.php" class="link-primary">To ship</a>&nbsp&nbsp
                        <a href="shippingOrder.php" class="link-primary">Shipping</a>&nbsp&nbsp
                        <a href="completedOrder.php" class="link-primary">Completed</a><br>
                    <?php
                    // Attempt select query execution
                    $query ="SELECT * FROM `order` WHERE sellerID = '".$sellerID."' ORDER BY orderID DESC";
                                    ...
                                    if($resultList) {
                                        ...
                                        while($product = mysqli_fetch_assoc($resultList)) {
                                            ...
                                            if($i<1) {
                                                ...
                                                echo "<td rowspan='$count'>".$product['paymentStatus']."</td>";
                                                $sn++;
                                                // if(is_null($product['deliveryID'])){
                                                //     echo "<td rowspan='$count'> - </td>";
                                                // } else {
                                                //     $deliveryQuery = "SELECT * FROM `delivery`,`courier`
                                                //     WHERE `delivery`.courierID = `courier`.courierID AND 
                                                //     deliveryID = ".$product['deliveryID'].";";
                                                //     $resultDelivery = mysqli_query($conn,$deliveryQuery);
                                                //     if($resultDelivery){
                                                //         while($deli = mysqli_fetch_assoc($resultDelivery)){
                                                //             echo "<td rowspan='$count'>".$deli['trackingNumber']."<br>".$deli['name']."<br>".$deli['shippingDate']."</td>";
                                                //         }
                                                //     }
                                                // }
                                               
                                                //echo "<td rowspan='$count'>".$row['addressID']."</td>";
                                                //echo "<td rowspan='$count'>".$row['orderID']."</td>";
