...
$name = $phoneNo = $email = $username = $password = $pin = "";
$name_err = $phoneNo_err = $email_err = $username_err = $password_err = $pin_err = "";
...
if(isset($_POST["customerID"]) && !empty($_POST["customerID"])){
    ...
    if(empty($name_err) && empty($phoneNo_err) && empty($email_err) && empty($username_err) && empty($password_err) && empty($pin_err)){
        // Prepare an update statement
        $sql = "UPDATE customer SET name=?, phoneNo=?, email=?, username=?, password=?, pin=? WHERE customerID=?";

        if($stmt = mysqli_prepare($conn, $sql)){
            // Bind variables to the prepared statement as parameters
			//sssi = string,string,string,integer
			// might change datatype tu varchar in database
            mysqli_stmt_bind_param($stmt, "ssssssi", $param_name, $param_phoneNo, $param_email, $param_username, $param_password, $param_pin, $param_customerID);
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <a href="custUser.php" class="btn btn-secondary ml-2">Cancel</a>
                    </form>
                </div>
            </div>        
        </div>
